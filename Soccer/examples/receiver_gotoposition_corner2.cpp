...
PlayerTask player_plan(const WorldModel* model, int robot_id){
	...
	point2f templeft;
	templeft.x = ball_pos.x - 20;
	templeft.y = ball_pos.y - 30;
	point2f tempright;
	tempright.x = ball_pos.x - 20;
	tempright.y = ball_pos.y + 30;
	point2f kicker_temp(170, 0);
	task.orientate = angle_atob(player_pos, ball_pos);
	task.target_pos = polarcoor_by_origin(ball_pos, 8, angle_atob(kicker_temp, ball_pos));
	if (length_atob(ball_pos, player_pos) < 30)
	{
		task.needCb = true;
	}
	task.orientate = angle_atob(player_pos, kicker_temp);
	return task;
}
